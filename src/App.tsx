...
export default function App() {
  ...
  const pageCollisionRegions: FixedRegion[] = useMemo(
    () => [
      ...pageRegions,
      ...collectExtendedColliders(ENTITIES, positions, activeWidget),
    ],
  ...
  const minimapShapes = useMemo<MinimapShape[]>(() => {
    const shapes: MinimapShape[] = PAGES.filter(
      (p) => p.id !== "header-zone",
    ).map((p) => {
  ...
  const handleEntityPositionChange = useCallback(
    (id: string, x: number, y: number) => {
      setPositions((prev) => {
        ...
        const widgetEntity = ENTITIES.find(
          (e) => e.id === id && e.category === "widget",
        );
  ...
  // ─────────────────────────────────────────────────────────
  // Two fingers down → pinch zoom + pan, anchored so the canvas point under
  // the gesture's starting midpoint stays under the fingers.
  // Single-finger touch keeps falling through to the pointer-event pan path.
  useEffect(() => {
    const viewport = viewportRef.current;
    if (!viewport) return;
    ...
    const handleTouchMove = (e: TouchEvent) => {
      ...
      const newZoom = pinch.baseZoom * ratio;
      // Keep the canvas point that was under the gesture's start midpoint
      // under the current midpoint at the new zoom.
      const newPanX =
        mid.x - ((pinch.startMidX - pinch.basePanX) / pinch.baseZoom) * newZoom;
      const newPanY =
        mid.y - ((pinch.startMidY - pinch.basePanY) / pinch.baseZoom) * newZoom;
          ...
          {entitiesWithPinState.map((entity) => {
            ...
            return (
              <HandwritingEntity
                ...
                renderSection={
                  entity.category === "section" ? renderSection : undefined
                }
                isWidgetActive={
                  entity.category === "widget"
                    ? activeWidget === entity.id
                    : undefined
                }
                onWidgetActivate={
                  entity.category === "widget"
