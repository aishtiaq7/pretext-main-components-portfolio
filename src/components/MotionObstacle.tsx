...
const VELOCITY_STOP_THRESHOLD = 0.0008 // canvas-pct per ms
...
// MotionObstacle — framer-motion–polished draggable entity that also acts as
// a text-reflow obstacle. Supports both IMAGE content (rocket SVG, etc.) and
// TEXT content (red handwritten words like "deadline", "ASAP").
//
...
export function MotionObstacle({ entity, x, y, zoom, onPositionChange }: Props) {
  ...
  // Dimensions: use image size if present, else obstacle bounding box
  const w = entity.imgW ?? entity.obstacleW ?? 100
  const h = entity.imgH ?? entity.obstacleH ?? 40
  ...
  return (
    <motion.div
      ...
      whileHover={{ scale: 1.08, rotate: (entity.rotate ?? 0) + 4 }}
      whileTap={{ scale: 0.94 }}
