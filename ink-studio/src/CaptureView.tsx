...
// ═══════════════════════════════════════════════════════════
// Capture view — record pen strokes with pressure/tilt/timing
// and export as JSON (for replay) or SVG (for static use).
//
// Draw on the left stage; tweak options in the right sidebar.
// Export → drop the file into the portfolio's public/handwriting/.
//
// Keyboard shortcuts (any view):
//   Space — toggle the recording transport (record / pause / resume)
//   C     — clear the stage
...
export function CaptureView() {
  ...
  }, [fillColor, outlineColor, outlineWidth, opacity, options, localPoint, recNow])
  ...
  const buildSvgString = (): string => {
    ...
    const paths = strokes.map((s) => {
      ...
      const opacity = op < 1 ? ` opacity="${op}"` : ''
      return `  <path d="${d}" fill="${s.color}"${stroke}${opacity} />`
    }).join('\n')
    return `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 ${rect.width} ${rect.height}" width="${rect.width}" height="${rect.height}">\n${paths}\n</svg>`
  }

  // ── Keyboard shortcuts ───────────────────────────────
  // Space cycles the transport (idle → recording → paused → recording …).
  // C clears the stage. ⌘/Ctrl-Z undoes the last stroke.
  // The transport cycle deliberately does nothing in 'stopped' — the user
  // must Clear before drawing again, mirroring the on-screen button states.
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      if (e.code === 'Space') {
        ...
        else if (s === 'recording') pauseRecording()
  ...
  }, [undo, startRecording, pauseRecording, resumeRecording, clearAll])
  ...
  return (
    <div className="app">
      ...
      <div ref={stageRef} className={`stage stage-${recStatus}`}>
        ...
        <div className="stage-transport" role="toolbar" aria-label="Recording transport">
          <div className={`clock clock-${recStatus}`} aria-live="polite">
            ...
            <span className="clock-time">{formatClockMs(clockMs)}</span>
          ...
          <div className="transport-buttons">
            ...
            <button
              ...
              className="transport"
              title={recStatus === 'paused' ? 'Resume (Space)' : 'Pause (Space)'}
            >
        ...
        <div className="metrics">
          pointer: {metrics.pointerType} • pressure: {metrics.pressure.toFixed(2)} • tilt: {metrics.tiltX.toFixed(0)},{metrics.tiltY.toFixed(0)} • strokes: {strokes.length} • <kbd>Space</kbd> transport · <kbd>C</kbd> clear
        </div>
      </div>
      ...
      <aside className="sidebar">
        ...
        <Slider label="Opacity" value={opacity} min={0} max={1} step={0.05}
                onChange={setOpacity} />
        ...
        <hr style={{ border: 'none', borderTop: '1px solid var(--divider)', margin: '14px 0 10px' }} />
        <RouteNav active="capture" className="route-nav-sidebar" />
      </aside>
    </div>
  )
}
